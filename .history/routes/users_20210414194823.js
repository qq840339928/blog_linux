...
router.get('/userInfo',async(req,res,next) => {
  ...
  try {
    let userinfo = await query('select nickname,headportrait from user where id = ?',[req.query.id])
...
router.post('/upload',upload.single('head_img'),async(req,res,next) => {
  ...
  res.send({code:200,msg:'上传成功',data:{ headportrait: imgUrl}})
})

//用户信息更新接口
router.post('/updateUser',async(req,res,next) => {
  let {nickname,headportrait, id} = req.body
  console.log(req.body)
  try {
    let result = await query('update user set nickname = ?,headportrait = ? where id = ?',[nickname,headportrait,id])
    console.log(result)
