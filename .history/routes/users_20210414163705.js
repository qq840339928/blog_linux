...
router.get('/info',async(req,res,next) => {
  ...
  try {
    let userinfo = await query('select nickname,headportrait from user where username = ?',[username])
...
router.post('/updateUser',async(req,res,next) => {
  ...
  try {
    let result = await query('update user set nickname = ?,head_img = ? where username = ?',[nickname,headportrait,id])
    console.log(result)
