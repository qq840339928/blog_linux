...
router.post('/add', async(req, res, next) => {
  ...
  try {
    ...
    await querySql('insert into blog(title,content,user_id,publishDate) values(?,?,?,NOW())',[title,content,user_id])
...
router.post('/update', async(req, res, next) => {
  ...
  try {
    ...
    let user = await querySql(userSql,[username])
    let user_id = user[0].id
    let sql = 'update blog set title = ?,content = ? where id = ? and user_id = ?'
...
router.post('/delete', async(req, res, next) => {
  ...
  try {
    ...
    let user = await querySql(userSql,[username])
    let user_id = user[0].id
    let sql = 'delete from blog where id = ? and user_id = ?'
