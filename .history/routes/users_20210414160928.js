...
router.post('/login', async function(request, response, next) {
  ...
  try {
    ...
    responseText = {code: 200, msg: '登陆成功', token:token, data:{nickname: dbUser[0].nickname || '张三', headportrait: dbUser[0].headportrait}}
    // 把token写入客户端的cookie中
    response.send(responseText)
  } catch (error) {
    console.log(error)
...
router.post('/upload',upload.single('head_img'),async(req,res,next) => {
  ...
  // 将头像地址存到数据库中
  let dbResult = await query('update user set headportrait = ? where username = ?', [imgPath, user.username])
  console.log(dbResult, '1433223')
