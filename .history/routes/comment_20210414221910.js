...
router.get('/list',async(req,res,next) => {
    ...
    try {
      ...
      result = await result.map(async element => {
        // 查找这个用户的昵称和头像
        userinfo = await querySql('select nickname,headportrait from user where id = ?',[element.user_id])
        element.nickname = userinfo[0].nickname
        element.headportrait = userinfo[0].headportrait
        console.log(element)
        return element
      })
